...
A Homie-style worker policy with the following enhancements: (1) modifications based on [this issue](https://github.com/InternRobotics/OpenHomie/issues/16), and (2) addition of locomotion training in semi-crouched poses, expanding beyond standing-only movement training.
...
```
1. conda create -n worker python=3.8
2. conda activate worker
3. cd path_to_downloaded_isaac_gym/python
4. pip install -e .
```
B. Install this repository:
```
1. cd Worker && pip install -r requirements.txt
...
```
export LD_LIBRARY_PATH=path_to_miniconda3/envs/worker/lib:$LD_LIBRARY_PATH
```
### Play
